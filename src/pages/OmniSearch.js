...
export default function OmniSearch(props) {
  ...
  useEffect(() => {
    getCategoriesForOmni().then((resp) => {
  ...
  const checkBoxHandleChange = (event, obj) => {
    ...
    for (let x = 0; x < filter_obj.groupBy.values[0].groupBy.values.length; x++) {
        ...
        <Grid container spacing={3} style={{ marginTop: 20 }}>
          <Grid item md={12} sm={12}>
            <div className="searchInputFormComMain">
              <form className="formControlClass" onSubmit={(event) => searchFunOfAsst(event)}>
                <div
                ...
                >
                  <TextField
                    ...
                    label="Search"
                    variant="outlined"
                    onChange={(event) => { setSearchingTxt(event.target.value) }}
                  />
                </div>
                <div className="btnControlClass">
                  <select name="setSelectedAsset" id="selectOneVel" onChange={(event) => { setSelectedAsset(event.target.value) }}>
                  ...
                  <Button type="submit" className="searchBtn" disabled={!searchingTxt}>Search</Button>
                </div>
              </form>
              {checkBoxListObj.groupBy &&
                <div>
                  <div className="filterChipMain">
                    {checkBoxListObj?.groupBy?.values[0].groupBy.values.map((checkObj, indx) => {
                      return (<div key={indx} className="ChipMain">
                        <span>
                          <FormGroup row>
                            <FormControlLabel
                              control={
                                <Checkbox
                                  ...
                                  name="checkedI" checked={checkObj.name == slctdAsstType} onChange={(event) => checkBoxHandleChange(event, checkObj)}
                                />
                              }
                              label={checkObj.name}
                            />
                ...
                <Grid container spacing={3} style={{ marginTop: 20 }}>
                  {filteredResults.map((snglObj, index) => {
                    return (<Grid key={index} item md={4} sm={12}>
                      <div className="filteringResults" onClick={onClickPageChange}>
            ...
            {(secViewName == "loader") && <div style={{ textAlign: "center", padding: "50px" }}><img width="100px" src={mainLoader} alt="Loader" /></div>}
            {(secViewName == "error") && <div style={{ textAlign: "center", padding: "50px" }}>Doesn't look like any data is available</div>}
            {(secViewName == "") &&
              <div style={{ textAlign: "center", padding: "50px" }}><img src={searchIcon} alt="Search" width="500px" /></div>
            }
          </Grid>
        </Grid>
      </LayoutContainer>
